...
class TestCd:
    def test_nonexistent_dir(self):
        with patch('src.cd.os.path.exists') as mock_exists:
    ...
    def test_file_instead_dir(self):
        with patch('src.cd.os.path.exists') as mock_exists, \
             patch('src.cd.os.path.isdir') as mock_isdir:
    ...
    def test_absolute_path(self):
        with patch('src.cd.os.path.exists') as mock_exists, \
             patch('src.cd.os.path.isdir') as mock_isdir, \
             patch('src.cd.os.path.abspath') as mock_abspath, \
             patch('src.cd.os.chdir') as mock_chdir:
            mock_exists.return_value = True
    ...
    def test_home_dir(self):
        with patch('src.cd.os.path.expanduser') as mock_expanduser, \
             patch('src.cd.os.path.exists') as mock_exists, \
             patch('src.cd.os.path.isdir') as mock_isdir, \
             patch('src.cd.os.chdir') as mock_chdir:
